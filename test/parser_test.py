...
def test_scan():
    ...
    assert str(last["text"]).startswith(
        "Ontario's 16-month-old government announced its first budget last spring "
    )
    assert (
        last["guid"]
        == "http://podcasts.tvo.org/theagenda/video/2587210_11911071710.mp4"
    )

    filename = "test/data/IntelligenceSquared.xml"
    items = scan(filename, userstring)
    assert len(items) == 4
    assert str(items[2]["text"]).endswith(
        "psychologist.  For information regarding your data privacy, visit acast.com/privacy"
    )


def test_nominal_functionality():
    p = parser.Parser()
    p.parse("test/data/IntelligenceSquared.xml")
    episodes = p.episodes()
    assert len(episodes) == 360
    assert (
        episodes[len(episodes) - 1].title == "Intelligence Squared Presents the Elders"
    )
